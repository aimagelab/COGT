...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None,
                 vdim=None, lora_alpha: int = 1, r=0, lmbda: bool = False):
        ...
        self.lora_lmbda = nn.parameter.Parameter(torch.zeros(num_heads)) if lmbda else None
        self.lora_la = nn.parameter.Parameter(torch.ones(num_heads)) if lmbda else None
        self.lora_lb = nn.parameter.Parameter(torch.zeros(num_heads)) if lmbda else None
    ...
    Shape:
        ...
        if not self._qkv_same_embed_dim:
            return multi_head_attention_forward(
                ...
                self.in_proj_weight, self.in_proj_bias, self.in_proj_weight_lora_A, self.in_proj_weight_lora_B,
                self.scaling,
                self.bias_k, self.bias_v, self.add_zero_attn,
                self.dropout, self.out_proj.weight, self.out_proj.bias, self.out_proj.lora_A, self.out_proj.lora_B,
                ...
                q_proj_weight_A=self.q_proj_weight_lora_A, k_proj_weight_A=self.k_proj_weight_lora_A,
                v_proj_weight_A=self.v_proj_weight_lora_A,
                q_proj_weight_B=self.q_proj_weight_lora_B, k_proj_weight_B=self.k_proj_weight_lora_B,
                v_proj_weight_B=self.v_proj_weight_lora_B,
                q_proj_weight_scaling=self.scaling, k_proj_weight_scaling=self.scaling,
...
def multi_head_attention_forward(query: Tensor,
    ...
    if attn_mask is not None:
        if attn_mask.dtype == torch.bool:
            ...
            # attn_mask = causal mask (CLIP default text encoder) size [77, 77]
